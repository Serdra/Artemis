...
void PUCTTree::backup(Path &path, float value) {
    ...
    if (path.size > 0) {
        ...
        if (value == MATE_VALUE) {
            tree[leaf_idx].setSolution(MATE_VALUE);
    ...
    for(int i = path.size - 1; i >= 1; i--) {
        ...
        for (int child_offset = 0; child_offset < node.num_children; child_offset++) {
            ...
            if (child.isSolved()) {
                ...
                if (child_solution == -MATE_VALUE) { // Child is a loss (win for us)
                    has_losing_child = true;
                }
                if (child_solution != MATE_VALUE) { // Child is not a win
